...
class RepositoriesEndpointTest {
    ...
    @Autowired
    RepositoriesEndpointTest(TestRestTemplate aTestRestTemplate, GithubUserService aGithubUserService ) {
    ...
    void httpStatusShouldBe200WhenProperRequest() {
        ResponseEntity<String> responseEntity = this.restTemplate
                .getForEntity("http://localhost:" + port + "/" + PROPER_USER_NAME+ "/repositories", String.class);
    ...
    void jsonsShouldBeTheSameWhenProperRequest() {
        ...
        assertEquals(responseEntity.getBody(),gson.toJson(githubUserService.getRepositoriesWithStars(PROPER_USER_NAME)));
    }

    @Test
    void httpStatusShouldBe500WhenRequestWithNonExistingUser() {
        ResponseEntity<String> responseEntity = this.restTemplate
                .getForEntity("http://localhost:" + port + "/" + NON_EXISTING_USER_NAME + "/repositories", String.class);
