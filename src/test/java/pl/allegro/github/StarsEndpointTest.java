...
class StarsEndpointTest {
    ...
    @Autowired
    StarsEndpointTest(TestRestTemplate aTestRestTemplate, GithubUserService aGithubUserService ) {
    ...
    void httpStatusShouldBe200WhenProperRequest() {
        ResponseEntity<String> responseEntity = this.restTemplate
                .getForEntity("http://localhost:" + port + "/" + PROPER_USER_NAME+ "/stars", String.class);
    ...
    void jsonsShouldBeTheSameWhenProperRequest() {
        ...
        json.addProperty("star_count",githubUserService.getTotalStarsAmount(PROPER_USER_NAME));

        assertEquals(responseEntity.getBody(),gson.toJson(json));
    }

    @Test
    void httpStatusShouldBe500WhenRequestWithNonExistingUser() {
        ResponseEntity<String> responseEntity = this.restTemplate
                .getForEntity("http://localhost:" + port + "/" + NON_EXISTING_USER_NAME + "/stars", String.class);
